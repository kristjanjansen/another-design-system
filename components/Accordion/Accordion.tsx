...
import { IconChevronDownAltSm } from "../../icons";
...
const Accordion = (props: AccordionProps) => {
  ...
  const handleVisibilityToggle = () => {
    if (!stateOrNull) {
      ...
      if (mode === "single") {
        selectedKeys?.includes(id) && (setSelectedKeys([]), setOpen(false));
        !selectedKeys?.includes(id) && (setSelectedKeys([id]), setOpen(true));
      }
  ...
  return (
    <div className={accordionClassList}>
      <Heading variant="h5" as="h3" className={styles.accordion__header}>
        ...
        >
          <span className={iconClassList}>
            <IconChevronDownAltSm aria-hidden={true} />
          </span>
          {title}
        </button>
      </Heading>
      ...
      <div
        ...
        aria-hidden={!open}
      >
        <div className={styles["accordion__panel-wrapper"]} ref={wrapperRef}>
          {children}
        </div>
      </div>
    </div>
